...
class LazySupervisedDataset(Dataset):
    ...
    def process_image_unified(self, image_file):
        ...
        return image_tensor, grid_thw, true_image
        ...
        # image_folder = sources[0]["data_path"]
            ...
            if isinstance(image_file, List):
                if len(image_file) > 1:
                    ...
                    image, grid_thw, true_image = zip(*results)
                else:
                    image_file = image_file[0]
                    image_file = os.path.join(self.img_dir, image_file)
                    image, grid_thw, true_image = self.process_image_unified(image_file)
                    image = [image]
            else:
                image_file = os.path.join(self.img_dir, image_file)
                image, grid_thw, true_image = self.process_image_unified(image_file)
